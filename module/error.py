...
class Cog(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error,commands.MissingRequiredArgument):
            ...
            arg = re.split('[.,:]',arg)
            embed = discord.Embed(title="引数不足です！",description=f"引数``{arg[0]}``が足りていません！",color=discord.Colour.from_rgb(255,0,0))
            try:
                desc = varname[arg[0]]
                embed.add_field(name=f"💡もしかしたら...",value=f"``{desc}``が不足していませんか？")
            except:
                pass
            ...
            embed = discord.Embed(title=f'取得に失敗しました！',description=f"引数の``{target}``を取得できませんでした！",color=discord.Colour.from_rgb(255,0,0))
            try:
                desc = target_dir[target]
                embed.add_field(name="💡もしかして...",value=f"引数の``{desc}``は実際に存在していますか？\n実際に存在しているオブジェクトでも、凜花が認識していないオブジェクトは取得できない場合があります。")
            except:
                pass
            await waiting.edit(content=f"{ctx.author.mention}->",embed=embed)
            return
